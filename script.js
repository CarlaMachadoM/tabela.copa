...
function load(){
  ...
  if(data){
    ...
    Object.keys(saved).forEach(group=>{
      saved[group].forEach((game,i)=>{
        if(matches[group] && matches[group][i]){
          matches[group][i].g1 = game.g1;
          matches[group][i].g2 = game.g2;
        }
      });
    });
