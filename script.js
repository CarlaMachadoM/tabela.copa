...
function load(){const d=localStorage.getItem("matches");if(d)Object.assign(matches,JSON.parse(d));}
...
function createGroup(name,games){
  games.sort((a,b)=>new Date(`2026-${a.date.split("/").reverse().join("-")}T${a.time}`)-new Date(`2026-${b.date.split("/").reverse().join("-")}T${b.time}`));
  ...
  html+=`<button class="clear-btn" onclick="clearGroup('${name}')">Limpar</button>
...
function generateKnockout(){
  ...
  Object.keys(matches).forEach(g=>{
    const res=calc(g);
    if(res.length>=2) classificados.push(res[0].time,res[1].time);
  });
